...
class PointConnectInput(ft.DragTarget):
    def __init__(self):
        super().__init__(content=LayoutBuilder(ft.Container(
                    width=20,
                    height=20,
                    shape=ft.BoxShape.CIRCLE,
                    bgcolor='red',
    ...
    def accept(self, e: ft.DragTargetAcceptEvent):
        ...
        end_x = self.content.x
        end_y = self.content.y
...
class PointConnectInput(ft.LayoutBuilder):
    def __init__(self):
        super().__init__()
        self.update_size_on_init = True
...
class Node(ft.GestureDetector):
    ...
    def __init__(self, top=0, left=0,on_update=None,on_drag_point=None):
        ...
        self.point_connect_in_1 = PointConnectInput()
        ...
        self.content = ft.Stack([
            self.block_draw,
            ft.Column([ft.Row([self.point_connect_in],alignment=ft.MainAxisAlignment.START),
                       ft.Row([self.point_connect_in],alignment=ft.MainAxisAlignment.START)
...
def main(page: ft.Page):
    def on_click(e):
        print(lol.layout_size)
    page.update()
    lol = ft.LayoutBuilder(
        content=ft.Container(expand=True,bgcolor='red',alignment=ft.alignment.center,content=ft.Text('lol')),
    ...
    page.add(lol)
    page.add(ft.ElevatedButton(text='lol',on_click=on_click))
    page.update()
